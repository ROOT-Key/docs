...
```
Systems under audit (SIEM, ITSM, application events)
        │
        │  On each auditable event:
        ▼
  ROOTKey API  ──── POST /files or /records
        │
        ▼
  ...
  └──────────────────────────────────┘
        │
        ▼
  Audit log: tamper-evident, third-party verifiable
  ← queryable at any time via ROOTKey API or Polygonscan
```
