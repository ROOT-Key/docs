...
Field devices / applications
        │
        ▼ (MQTT or REST)
ROOTKey Ingestion Layer  ──────────────────────────────────────────
(OVH - EU jurisdiction)
        │
        ├──► Off-chain integrity record (OVH - EU jurisdiction)
        │    └── SHA-256 hash + timestamp + device/source ID
        │
        └──► No blockchain transaction - high-frequency operation
             maintained at scale
