...
```
Customer on-premise infrastructure (EU)
┌──────────────────────────────────────────────────────────────┐
│                                                              │
│  Field devices / applications                               │
│        │                                                    │
│        ▼ (MQTT or REST - internal only)                     │
│  ROOTKey Container (deployed on-premise)                    │
│        │                                                    │
│        ├──► Off-chain integrity records (on-premise storage) │
│        │    └── SHA-256 hash + timestamp + device ID        │
│        │                                                    │
│        └──► Selective EBSI anchoring (outbound, optional)   │
│             └── For audit-grade records: alarms, incidents, │
│                 threshold breaches, regulatory measurements │
│                                                              │
└──────────────────────────────────────────────────────────────┘
  All data remains within customer perimeter.
  EBSI connection is the only external dependency (optional).
```
