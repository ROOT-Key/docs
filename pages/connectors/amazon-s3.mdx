...
The S3 connector is a self-contained integration that runs **entirely inside your AWS account**. ROOTKey publishes a [Terraform module](https://github.com/ROOT-Key/rootkey-connectors/tree/main/aws-s3) that you deploy once. The module wires up a Lambda function to receive Amazon EventBridge events emitted by your S3 bucket whenever a new object is created.
...
<Steps>
  ...
  <Step title="Copy the Connector API Key and the Terraform block">
    ...
    module "rootkey_s3_connector" {
      # Pinned to a release tag. Without a ?ref= the source resolves to whatever
      # is on the default branch the moment you run terraform init, so two
      # deployments a week apart can get different code. Most change-control
      # processes will require the pin.
      ...
      # "Secrets and the Terraform state file" below.
...
Your ROOTKey API Key is written to Secrets Manager using `secret_string_wo`, a **write-only argument**. The provider receives the value, sends it to AWS Secrets Manager, and Terraform persists nothing. The value is equally absent from a saved plan file (`terraform plan -out=…`), because the corresponding input variables are declared `ephemeral` — worth knowing if you run the module from a CI/CD pipeline, where a saved plan is exactly the kind of thing that gets archived as a build artifact.

You can verify this yourself after an apply. The following should return `0`:

```bash
grep -c "<the secret value you supplied>" terraform.tfstate
```

### The trade-off: Terraform cannot detect that a secret changed

It never sees the value, so it has nothing to compare against. That is what `rootkey_api_key_version` is for.
...
Being complete about this matters more than the headline:

| What | Why it is there |
|---|---|
| Resource ARNs, names, IAM policy documents | Infrastructure metadata. The Lambda's `environment` block holds the secret's **ARN**, never its value — the function reads the value from Secrets Manager at cold start using its execution role. |
| CloudWatch log group and SQS queue names | Infrastructure metadata. |

None of these is a credential to your tenant or account, but list them anyway if you are producing an inventory for an audit.
...
The trust boundary is the one you already accepted when you let the module create AWS Secrets Manager inside your own account. If that is acceptable, the state file is acceptable in the same place.
