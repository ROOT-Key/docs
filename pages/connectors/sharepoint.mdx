...
<Steps>
  ...
  <Step title="Copy the Connector API Key and the Terraform block">
    ...
    module "rootkey_sharepoint_connector" {
      ...
      # Rotation counters. Increment the matching counter whenever you change
      # the secret above — see "Secrets and the Terraform state file" below.
...
|---|---|
| The webhook `clientState` | A validation token the module generates so the Function App can reject forged Microsoft Graph notifications. It is not a credential to your tenant. It stays in state because it has to be stable across applies — regenerating it on every apply would invalidate notifications already in flight. |
| The Application Insights connection string | Contains the instrumentation key. It grants telemetry ingestion and nothing else. |
