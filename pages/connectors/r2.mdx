...
The R2 connector is a self-contained integration that runs **entirely inside your Cloudflare account**. ROOTKey publishes a [Terraform module](https://github.com/ROOT-Key/rootkey-connectors/tree/main/r2) that you deploy once per bucket. The module wires a Cloudflare Worker to a Cloudflare Queue that receives R2 event notifications whenever a new object is created.
...
| `secret_text` binding on the Worker | Holds the ROOTKey Connector API Key. Encrypted at rest by Cloudflare; **never readable back** through the dashboard or API after creation. Provider v5 removed the standalone `cloudflare_workers_secret` resource — a binding on the Worker script is its replacement, and is what the old resource compiled down to anyway. | None (free with Workers). |
...
<AccordionGroup>
  ...
  <Accordion title="What happens to my API key?" icon="key">
    The Connector API Key you paste into Terraform is delivered to the Worker as a `secret_text` binding **in your own account**, encrypted at rest by Cloudflare. After creation it cannot be read back through the dashboard or API — only overwritten via `terraform apply` with a new value. The Worker reads it from `env.ROOTKEY_API_KEY` at runtime.

    It is, however, written to your Terraform state file — unlike the SharePoint, OneDrive and Amazon S3 connectors. Read [Secrets and the Terraform state file](#secrets-and-the-terraform-state-file) below before deciding where that state lives.
  </Accordion>
  ...
  <Accordion title="Can I roll this back?" icon="rotate-left">
    Yes. Running `terraform destroy` removes every resource the module created (Worker and its bindings, both queues, queue consumer, event notification). The R2 bucket itself is **not** deleted — it is your resource, not the module's.
  </Accordion>
</AccordionGroup>
...
- [Terraform](https://developer.hashicorp.com/terraform/install) **v1.11 or later**. Pinned to the same floor as the other three connectors so there is one Terraform version to support across the set.
...
<Steps>
  ...
  <Step title="Copy the Connector API Key and the Terraform block">
    ...
    module "rootkey_r2_connector" {
      source = "github.com/ROOT-Key/rootkey-connectors//r2?ref=v1.0.1"
...
- **API key in a `secret_text` binding, not in code.** Encrypted at rest by Cloudflare, not readable back after creation. Rotation = `terraform apply` with a new value. It does reach your Terraform state file, though — see [Secrets and the Terraform state file](#secrets-and-the-terraform-state-file).
...
Terraform records the attributes of everything it manages in a state file. By default that includes the *value* of any secret passed into a module — marking a variable `sensitive` only masks it in command output, it does not keep it off disk.

<Warning>
  **This connector behaves differently from the other three.** In the SharePoint, OneDrive and Amazon S3 modules your secrets are never written to state: those providers support *write-only arguments*, so the value reaches the vault and Terraform persists nothing.

  The Cloudflare provider implements no equivalent, so `rootkey_api_key` **is** persisted to `terraform.tfstate`.
</Warning>

This is a limitation of the provider, not a choice of this module. It was verified against the schema the pinned provider version actually ships — the check reports `0` across all 264 resources of cloudflare/cloudflare v5.25.0:
...
Worth stating plainly what follows from it:
...
- There is consequently no `rootkey_api_key_version` rotation counter here, and the variable is not `ephemeral`. Both exist in the other three connectors purely to serve write-only arguments.
- The upside: Terraform *can* see this value, so rotation is a single step — change `rootkey_api_key` and apply. None of the silent-no-op sharp edge the other three connectors carry.

You can confirm the exposure yourself after an apply. Unlike the other three connectors, this returns a non-zero count:
...
|---|---|
| `rootkey_api_key`, as a Worker binding | The provider limitation described above. |
| The compiled Worker bundle | The built `dist/index.js`, read in so it can be uploaded. Not secret — it is the source published in our public repository. |
| Queue IDs and names, the Worker name, the event-notification config | Infrastructure metadata, not credentials. |

### What to do about it

Treat the state file at the same classification as the API key itself, and keep it off operator laptops. R2 works as an S3-compatible backend, which keeps the state in the same account as the rest of this deployment:
...
The backend authenticates with an **R2 API token** — created under R2 → Manage API tokens, with Object Read & Write on the state bucket. It is a different token from the Cloudflare API token the module itself uses. Supply it through the environment rather than writing it into the block, which would put a second credential into a file you commit:
...
<Note>
  Omitting these is the usual reason `terraform init` fails against an R2 backend with a signature or access-denied error. The `skip_*` flags above only suppress AWS-specific preflight calls that R2 does not implement — they do not make the backend anonymous.
</Note>

If your change-control process cannot accept a credential at rest in the state file at all, the alternative is to deploy this connector with an API key scoped to nothing else and rotate it on a schedule you control — the key grants upload rights to your own ROOTKey connector and nothing more.
