...
<Steps>
  <Step title="Unpack the release bundle">
    ...
    **Verify it before trusting anything inside it.** `sha256sum -c` alone proves the archive
    ...
    Or run the standalone verifier, which does both and tells you what a failure means:

    ```bash
    ./verify-bundle.sh rootkey-onprem-<version>.tar.gz
    ```

    See [Trust and Verification](/pages/on-premise/trust) for the public key, its fingerprint,
