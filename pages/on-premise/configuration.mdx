...
<Note>
  In Scenario C you also supply `vault_postgres_passwords` — five role passwords the installer applies to the database. It never generates its own, so re-running the install re-asserts exactly what your vault says rather than rotating anything behind your back.
</Note>
...
| `blockchain` | `RPC_URL`, `TESTNET_RPC_URL`, `WEB3_API_KEY`, `TESTNET_WEB3_API_KEY` | Where anchoring transactions are sent. |
...
Every one of the 26 workloads needs a real `repository` and `tag`. The tag is the released commit SHA of that service, which ROOTKey publishes with each release.
