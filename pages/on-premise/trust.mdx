...
<Steps>
  <Step title="Get the verifier and the public key from this page">
    Both are published here rather than only inside the bundle. A verifier that ships inside the
    thing it verifies answers the wrong question: if the archive had been tampered with, so had
    the script.

    ```bash
    curl -O https://docs.rootkey.ai/files/verify-bundle.sh
    curl -O https://docs.rootkey.ai/files/cosign.pub
    chmod +x verify-bundle.sh
    ```
  </Step>

  <Step title="Check the key fingerprint against your contract">
    The fingerprint below is also written into your SOW or contract. Two independent channels,
    so the verification is never circular: a page and a signature that both came from the same
    place prove nothing to each other.
    ...
    <Note>
      If the fingerprint on this page and the one in your contract disagree, stop and contact us
  ...
  <Step title="Verify the bundle">
    ```bash
    ./verify-bundle.sh rootkey-onprem-<version>.tar.gz
    ```

    It prints the archive's SHA-256, checks the cosign signature over `SHA256SUMS`, and confirms
    every file inside matches. It exits non-zero if any of those fail.
  </Step>

  <Step title="Compare the archive checksum against what we told you">
    The script prints the archive's SHA-256. Compare it against the value in the message that
    announced the release — **not** against a value that travelled with the file. A checksum
...
Superseded keys stay published below rather than being removed, so that a bundle you archived
years ago can still be verified against the key it was actually signed with.

<Note>
  **The current key fingerprint and the archive of superseded keys are published alongside this
  page.** If you are reading this before the key material has been attached, ask your ROOTKey
  contact for the fingerprint and check it against your contract — the two-channel rule applies
  either way.
</Note>
